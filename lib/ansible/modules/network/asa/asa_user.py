  ...
  aggregate:
    description:
      - The set of username objects to be configured on the remote
        Cisco ASA device. The list entries can either be the username
        or a hash of username and properties. This argument is mutually
        exclusive with the C(name) argument.
    aliases: ['users', 'collection']
  name:
    description:
      - The username to be configured on the Cisco ASA device.
        This argument accepts a string value and is mutually exclusive
        with the C(aggregate) argument.
        Please note that this option is not same as C(provider username).
  configured_password:
    description:
      - The password to be configured on the Cisco ASA device. The
...
from ansible.module_utils.network.asa.asa import get_config, load_config
    ...
    for update in updates:
        ...
        if needs_update(want, have, 'nopassword'):
            if want['nopassword']:
                ...
                add(commands, want, 'no username %s', want['name'])
...
def main():
    ...
    # the asa cli prevents this by rule so capture it and display
